...
function loadExercise() {
    ...
    exercise.options.forEach((option, index) => {
        ...
        // نزيل وسوم HTML للفحص فقط (لا نغيّر النص الفعلي)
        const optionPlainText = option.replace(/<[^>]*>/g, '');
        const hasMathContent = /[a-zA-Z\(\)\+\-\*\/=<>²³\d\/]/.test(optionPlainText);

        if (!isArabic && hasMathContent) {
            // خيار رياضي صرف: نضعه داخل span LTR مع الحفاظ على sup/sub/code
            optionBtn.setAttribute('data-pure-math', 'true');
            // نحوّل أولاً صيغة base^exp إلى <sup>، ونصلح ترتيب الجذور، ثم نزيل span/div فقط
            const safeOption = fixRadicalOrder(convertCaretToSup(option))
                .replace(/<(span|div)[^>]*>/gi, '')   // نزيل فتح span/div
                .replace(/<\/(span|div)>/gi, '');      // نزيل إغلاق span/div
            optionBtn.innerHTML = `<span dir="ltr" style="display:inline-block; font-family:'Courier New',monospace; font-weight:bold; unicode-bidi:isolate; font-size:1.05em;">${safeOption}</span>`;
        } else {
...
// ===== تحويل صيغة القوة النصية (base^exp) إلى أس حقيقي =====
function convertCaretToSup(text) {
    if (typeof text !== 'string') return text;
    if (text.includes('<sup>')) return text;
    return text.replace(/([\w\)\d]+)\^(-?\d+|\w+)/g, '$1<sup>$2</sup>');
}

// ===== إصلاح ترتيب الجذور =====
// يحوّل النمط الخاطئ N√ إلى √N صحيح، ويلفّه في <span dir="ltr"> للعرض السليم
// يتعامل مع: &radic;، √ (Unicode U+221A)
function fixRadicalOrder(text) {
    if (typeof text !== 'string') return text;
    // لا نعالج إذا كان HTML جاهز (span, div)
    if (text.includes('dir=')) return text;

    // ① تحويل N&radic; → <span dir="ltr">√N</span>   (جذر مكتوب بعد الرقم بـ &radic;)
    text = text.replace(/(\d+)&radic;/g, "<span dir='ltr'>√$1</span>");

    // ② تحويل N√ → <span dir="ltr">√N</span>   (جذر Unicode مكتوب بعد رقم بدون مسافة)
    text = text.replace(/(\d+)(√)/g, "<span dir='ltr'>√$1</span>");

    // ③ لف أي √N منفرد (جذر صحيح لكن بدون ltr) داخل span
    text = text.replace(/(√\d+)/g, "<span dir='ltr'>$1</span>");
...
function formatTextWithMath(text) {
    ...
    // 🔑 تحويل أي صيغة base^exp إلى <sup> قبل كل معالجة
    text = convertCaretToSup(text);

    // 🔑 إصلاح ترتيب الجذور المحتمل المعكوس
    text = fixRadicalOrder(text);
    ...
    exerciseContent.innerHTML = `
        <div style="text-align: center; padding: 40px; animation: scaleIn 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275);">
            ...
            <div style="display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
                ...
                <button onclick="window.top.location.href='index.html'" class="btn btn-success" style="background: linear-gradient(135deg, #48bb78, #38a169); border: none; padding: 12px 25px; font-size: 1.1rem; border-radius: 12px; box-shadow: 0 4px 15px rgba(72,187,120,0.4);">
...
function showCalculator() {
    window.location.href = 'calculatrice%20scientifique.html';
}
